...
Value getsubsidy(const Array& params, bool fHelp)
{
    ...
    return (uint64_t)GetPOSReward(nBestHeight, 0, 0);
}
...
Value getmininginfo(const Array& params, bool fHelp)
{
    ...
    obj.push_back(Pair("blockvalue",    (uint64_t)GetPOSReward(nBestHeight, 0, 0)));
...
Value checkkernel(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(-9, "RideMyCarCoin is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(-10, "RideMyCarCoin is downloading blocks...");
...
Value getwork(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "RideMyCarCoin is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "RideMyCarCoin is downloading blocks...");
...
Value getblocktemplate(const Array& params, bool fHelp)
{
    ...
    */
    ...
    uint256 hashTarget = CBigNum().SetCompact(pblock->nBits).getuint256();
    int64_t blockReward = pblock->vtx[0].vout[0].nValue + pblock->vtx[0].vout[1].nValue;
    ...
    result.push_back(Pair("coinbasevalue", blockReward));
